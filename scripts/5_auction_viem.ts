...
async function main() {
  ...
  // ================== 0) 转移10万个stable给custodian，确保有奖励资金 ==================
  console.log('0) 确保custodianFixed合约有足够奖励资金...');
  const rewardAmount = 10000n * 10n ** 18n;
  let custodianStable = await stable.read.balanceOf([CUSTODIAN as `0x${string}`]) as bigint;
  if(custodianStable >= rewardAmount)console.log('✅ Custodian already has enough stable.');
  else{
    console.log('🔶 0. 转移10,000 Stable给 Custodian 作为清算奖励资金...');
    const transferTx = await stable.write.transfer([CUSTODIAN as `0x${string}`, parseEther('10000')]);
    await publicClient.waitForTransactionReceipt({ hash: transferTx });
    // console.log('✅ Stable transferred to custodian.');
  }
  ...
  // 3) 检查是否触发清算条件：未触发则降低 oracle 价格(每次10%)至触发清算
  if(tokenIdUnderLiquidation == false) 
  {
    //没有处于清算，则手动调整价格，触发bark
    //处于清算，则跳过这一步
    console.log('3) 检查是否触发清算条件：未触发则降低 oracle 价格(每次10%)至触发清算...');
    ...
    console.log('Liquidation threshold:', formatEther(liquidationThreshold));//触发清算阈值为0.3, liquidationThreshold本身是18位
    ...
    // let tokenId = 8n; //想要拍卖清算的leverage token id
    // let auctionId = 0n;
    ...
    if (isUnder) {
      console.log('❌ 账户已触发清算，请勿重复操作');
      auctionId = (liqStatus as any).auctionId ?? (liqStatus as any)[7] ?? 0n;
      console.log(`Leverage token ID ${tokenId} is already under liquidation with auction ID: ${auctionId}.`);

    }
    else {
      console.log('✅ 账户未触发清算，准备bark...');
      try {
        const barkTx = await liquidationManager.write.bark([me, tokenId, me]);//在bark中已经burn掉相应的leverage token了
        const receipt = await publicClient.waitForTransactionReceipt({ hash: barkTx });
        console.log('✅ Bark completed, liquidation triggered.');
  ...
  console.log('5) 检查是否需要resetAuction，需要的话则reset...');
  ...
  console.log('7) 尝试 withdrawStable 提取清算所得...');
  try {
