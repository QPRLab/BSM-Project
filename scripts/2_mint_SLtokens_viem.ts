...
//   },
// ] as const;
// // CustodianFixed ABI (mint / getAllLeverageTokenInfo函数)
// const CUSTODIAN_ABI = [
//   {
//     inputs: [
//       { name: 'underlyingAmountInWei', type: 'uint256' },
//       { name: 'mintPriceInWei', type: 'uint256' },
//       { name: 'leverageLevel', type: 'uint8' },
//     ],
//     name: 'mint',
//     outputs: [
//       { name: 'sAmountInWei', type: 'uint256' },
//       { name: 'lAmountInWei', type: 'uint256' },
//       { name: 'leverageTokenId', type: 'uint256' },
//     ],
//     stateMutability: 'nonpayable',
//     type: 'function',
//   },
...
//     ],
//     name: 'getAllLeverageTokenInfo',
//     outputs: [
//         { name: 'tokenIds', type: 'uint256[]' },
//         { name: 'balancesInWei', type: 'uint256[]' },
//         { name: 'leverages', type: 'uint8[]' },
//         { name: 'mintPricesInWei', type: 'uint256[]' },
//         { name: 'accuredInterestsInWei', type: 'uint256[]' }
//     ],
...
// 铸币参数
const TOTAL_WLTC_AMOUNT = 500000n * 10n ** 18n; // 500000 WLTC
const MINT_AMOUNT = [245000n * 10n ** 18n, 245000n * 10n ** 18n, 10000n * 10n ** 18n]; // 分别给三种杠杆类型铸币的 WLTC 数量
...
async function main() {
  ...
  console.log(`\n🔓 Approve Custodian 使用 ${formatEther(TOTAL_WLTC_AMOUNT)} WLTC...`);
  const approveTx = await walletClient.writeContract({
    address: WLTC_ADDRESS,
    abi: WLTC_ABI,
    functionName: 'approve',
    args: [CUSTODIAN_ADDRESS, TOTAL_WLTC_AMOUNT],
  });
  ...
  for (const leverage of leverages) {
    console.log(`\n🎯 铸造 ${leverage.name} L token，使用 ${formatEther(MINT_AMOUNT[i])} WLTC...`);

    const mintTx = await walletClient.writeContract({
      address: CUSTODIAN_ADDRESS,
      abi: CUSTODIAN_ABI,
      functionName: 'mint',
      args: [MINT_AMOUNT[i], MINT_PRICE, leverage.type],
    });
    i++;

    console.log(`${leverage.name} 铸币交易哈希: ${mintTx}`);

    // 等待确认
    await publicClient.waitForTransactionReceipt({ hash: mintTx });
    console.log(`✅ ${leverage.name} 铸币完成`);
  }
  console.log('\n🎉 所有铸币完成！');
