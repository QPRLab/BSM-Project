...
contract AMMSwap is Ownable, ReentrancyGuard {
    ...
    /**
     * @notice USDC 到槓桿代幣的交換
     * @dev 用戶支付 USDC 購買指定數量的槓桿代幣，涉及 DEX 購買抵押物和 AMM 交換
     * 
     * 🔄 核心流程说明：
     * 
     * 资金流动：
     * 1. 用户支付部分 USDC → AMMSwap 合约
     * 2. AMMLiquidity 池转出部分 USDC → AMMSwap 合约（通过虚拟 S→USDC 交换）
     * 3. 总 USDC（用户 + AMM 池）→ DEX 购买 WLTC
     * 4. WLTC → Custodian 铸币 → 拆分成 S + L tokens
     * 5. L tokens → 用户
     * 6. S tokens → AMMLiquidity 池
     * 
     * AMM 池变化：
     * - USDC 减少: previewUsdcOut (步骤 2)
     * - S token 增加: actualSAmountInWei (步骤 6)
     * - 这两个变化必须满足 StableSwap 曲线方程
     * 
     * 原理：
     * 本质上是一个"虚拟交换" - 我们计算如果用 S tokens 换 USDC 能得到多少，
     * 然后提前取出这些 USDC 使用，稍后将铸造出的 S tokens 存入池子，
     * 最终池子的状态就像真的执行了 S→USDC 交换一样。
     *
     * 執行流程：
     * 1. 基本參數驗證
     * 2. 根據槓桿類型計算所需的 S token 和 underlying 數量
     * 3. 驗證計算結果合理性
     * 4. 計算從 AMM 池換取的 USDC 數量（使用 previewSwapStableToUsdc）
     * 5. 獲取 Oracle 價格並驗證
     * 6. 計算 L token 的淨值 (NAV)
     * 7. 計算總價值和所需 USDC（含滑點保護）
     * 8. 計算可用於購買 underlying 的總 USDC（用户 + AMM 池）
     * 9. 檢查 DEX 上購買 underlying 所需的 USDC
     * 10. 執行用戶 USDC 轉移到本合约
     * 10.5. ✅ 从 AMMLiquidity 提取 USDC 到本合约（关键步骤）
     * 11. 在 DEX 上購買 underlying
     * 12. 退還多餘 USDC 给用户
     * 13. 通過 Custodian 執行鑄幣操作（S tokens 直接存入 AMMLiquidity）
     * 14. 驗證鑄幣結果
     * 15. 更新 AMM 池狀態（同步储备，确保曲线平衡）
     * @dev 用户使用 USDC 购买杠杆代币的核心函数（重构版本）
     * 
     * 执行流程：
     * 1. 验证输入参数并计算需要的 underlying 数量
     * 2. 用户自己使用 USDC 在 DEX 上购买 underlying（WLTC）
     * 3. 用户将购买的 underlying 转入本合约
     * 4. 授权并调用 CustodianFixed.mintFromAMM 铸造 Stable 和 Leverage token
     * 5. 对于获得的 Stable token，通过 AMM 卖出换取 USDC
     * 6. 将获得的 USDC 退还给用户
     * 
     * 用户视角：付出 USDC → 获得 Leverage Token + 部分 USDC 退款
     * 
     * @param LAmountInWei 目标 L token 数量（18位精度）
     * @param mintPrice 铸造价格（18位精度，单位：USDC/underlying）
     * @param leverage 杠杆类型（CONSERVATIVE=0, MODERATE=1, AGGRESSIVE=2）
     * @param slippageTolerance DEX 交易滑点容忍度（基点，如 300 = 3%）
     * 
     * @return stableAmountMinted 实际铸造的 S token 数量（18位精度）
     * @return underlyingAmountInWei 购买的 underlying 数量（18位精度）
     * 
     * Requirements:
     * - LAmountInWei > 0
     * - mintPrice > 0
     * - slippageTolerance <= MAX_SLIPPAGE (1000基点 = 10%)
     * - 用户必须先在 DEX 购买足够的 underlying token
     * - 用户必须授权本合约使用其 underlying token
     * - AMM 池必须有足够的 USDC 储备用于购买 Stable
     * 
     * Emits:
     * - SwapUsdcToLeverage(user, netUsdcPaid, leverageTokenId, actualLAmountInWei)
     * - SwapStableToUsdc(user, stableAmountIn, usdcAmountOut) - 内部调用触发
     */
    function swapUsdcToLeverage(
        uint256 LAmountInWei,
        uint256 mintPrice,
        LeverageType leverage,
        uint256 slippageTolerance //输入100表示1%
    ) external nonReentrant returns (
        uint256 stableAmountMinted,
        ...
        // 根据杠杆类型计算所需的 S token 和 underlying 数量
        // 注意：向上取整以确保铸造足够的 token
        uint256 stableAmountRequired;
        if (leverage == LeverageType.CONSERVATIVE) {
            stableAmountRequired = LAmountInWei / 8;
            underlyingAmountInWei = (9 * stableAmountRequired * 1e18 + mintPrice - 1) / mintPrice;  // 向上取整
        } else if (leverage == LeverageType.MODERATE) {
            stableAmountRequired = LAmountInWei / 4;
            underlyingAmountInWei = (5 * stableAmountRequired * 1e18 + mintPrice - 1) / mintPrice;  // 向上取整
        } else if (leverage == LeverageType.AGGRESSIVE) {
            stableAmountRequired = LAmountInWei;
            underlyingAmountInWei = (2 * stableAmountRequired * 1e18 + mintPrice - 1) / mintPrice;  // 向上取整
        } else {
            revert("Invalid leverage level");
        }

        require(stableAmountRequired > 0, "Invalid stable amount calculated");
        require(underlyingAmountInWei > 0, "Invalid underlying amount calculated");

        // 获取 DEX 报价以估算用户需要准备的 USDC
        (uint256 requiredUsdcForUnderlying, uint256 dexPriceImpact) =
            this.getUsdcRequiredForUnderlying(underlyingAmountInWei);
        require(requiredUsdcForUnderlying > 0, "Failed to get DEX quote");
        require(dexPriceImpact <= 1000, "DEX price impact too high"); // 最大 10%

        // 计算用户需要准备的最大 USDC 数量（包含滑点）
        uint256 maxUsdcForDex = requiredUsdcForUnderlying * (10000 + slippageTolerance) / 10000;
        
        // 验证用户有足够的 USDC 余额和授权
        require(
            usdcToken.balanceOf(msg.sender) >= maxUsdcForDex,
            "Insufficient USDC balance"
        );
        require(
            usdcToken.allowance(msg.sender, address(this)) >= maxUsdcForDex,
            "Insufficient USDC allowance"
        );

        // ==================== 步骤 2：从用户转入 USDC，然后在 DEX 购买 WLTC ====================
        // 从用户转入 USDC 到本合约
        usdcToken.transferFrom(msg.sender, address(this), maxUsdcForDex);
        
        // 调用 buyUnderlyingOnDEX 在 DEX 上购买 WLTC
        uint256 actualUsdcSpent = buyUnderlyingOnDEX(
        ...
        // 退还多余的 USDC 给用户
        uint256 usdcRefund = maxUsdcForDex - actualUsdcSpent;
        if (usdcRefund > 0) {
            usdcToken.transfer(msg.sender, usdcRefund);
        }

        // ==================== 步骤 3：铸造 Stable 和 Leverage token ====================
        // 授权 CustodianFixed 使用 underlying token
        underlyingToken.approve(address(custodian), underlyingAmountInWei);
        
        // 调用铸币：S token 直接发送到本合约（避免需要用户 approve），L token 发送到用户
        (uint256 actualSAmountInWei, uint256 actualLAmountInWei, uint256 leverageTokenId) =
            custodian.mintFromAMM(
                address(this),          // S token 接收者：本合约（稍后转给 AMM）
                msg.sender,             // L token 接收者：用户
                underlyingAmountInWei,
                mintPrice,
                leverage
            );
        ...
        // 验证铸币结果
        require(actualSAmountInWei > 0, "Failed to mint S tokens");
        require(actualLAmountInWei >= LAmountInWei, "Insufficient L tokens minted");
        
        stableAmountMinted = actualSAmountInWei;

        // ==================== 步骤 4：将 Stable token 卖给 AMM 池换 USDC ====================
        // S token 现在在本合约地址，预览可以获得多少 USDC
        (
            uint256 previewUsdcOut,
            ,
            uint256 adminFeeUsdcAmount,
            uint256 lpFeeUsdcAmount,
            uint256 ammPriceImpact,
            bool isValidSwap
        ) = this.previewSwapStableToUsdc(actualSAmountInWei);
        
        require(isValidSwap, "Invalid AMM swap parameters");
        require(ammPriceImpact <= 5000, "AMM price impact too high"); // 最大 50%（测试环境）

        // 检查 AMM 的 USDC 储备
        (uint256 reserveStableBefore, uint256 reserveUsdcBefore) = ammliquidity.getReserves();
        uint256 totalUsdcRequired = previewUsdcOut + adminFeeUsdcAmount + lpFeeUsdcAmount;
        require(
            usdcToken.balanceOf(address(ammliquidity)) >= totalUsdcRequired,
            "Insufficient USDC in AMM pool"
        );

        // 从本合约地址转入 S token 到 AMM 池
        stableToken.transfer(address(ammliquidity), actualSAmountInWei);

        // 更新 AMM 池储备：增加 Stable，减少 USDC
        ammliquidity.syncReserves(
            reserveStableBefore + actualSAmountInWei,
            reserveUsdcBefore - (previewUsdcOut + adminFeeUsdcAmount)
        );

        // 从 AMM 池转出 USDC 给用户
        ammliquidity.transferUsdcToSwap(previewUsdcOut);
        usdcToken.transfer(msg.sender, previewUsdcOut);

        // 处理管理费
        ammliquidity.addAdminFee(0, adminFeeUsdcAmount);

        // ==================== 步骤 5：计算净支出和触发事件 ====================
        // 计算用户净支出的 USDC（购买 underlying 的花费 - 卖出 Stable 的收入）
        // netUsdcPaid = actualUsdcSpent - previewUsdcOut
        uint256 netUsdcPaid = actualUsdcSpent > previewUsdcOut 
            ? actualUsdcSpent - previewUsdcOut 
            : 0;
        ...
        return (stableAmountMinted, underlyingAmountInWei);
    }
